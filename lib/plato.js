...
plato.runReports = function(config){
  ...
  function gatherReports(file) {
    ...
    reportsAggregate[file] = {};

    function writeFile(source) {
      log.debug('Writing source for ' + file);
      // write source out to file for client retrieval
    ...
    function runReport(module) {
      ...
      promise.then(function(report){
        reportDetail[module] = report.detail || report;
        reportsAggregate[file][module] = report.aggregate || report;
      });
    ...
    function writeDetail() {
      ...
      var outfile = path.join(config.output, 'reports', safename + '-detail');
  ...
  function writeAggregate() {
    log.debug('Writing aggregate report');
    var outfile = path.join(config.output, 'reports', 'aggregate');
    var json = JSON.stringify(reportsAggregate);
    json = 'define(' + json + ');';
    return qfs.write(outfile + '.js', json);
  }

  var promises = config.files.map(gatherReports);

  Q.allSettled(promises).then(writeAggregate).then(deferred.resolve.bind({}, reportsAggregate),deferred.reject);
...
plato.runReport = function(module, options, futureFile){
  ...
  deferred.promise.then(function(report){
    plato.emit('report:generated', module, report);
  });
  return deferred.promise;
};
